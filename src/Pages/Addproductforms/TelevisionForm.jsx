...
const TabletForm = ({ category }) => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      const response = await axios.post('https://newkartbackend-1.onrender.com/addproducts/television/add', formData, {
        headers: {
  ...
  return (
    ...
    <input className="px-3 py-2 border border-gray-700 w-full"  {...register('technology', { required: true })} />
    </div>
    {errors.technology && <span>Technology is required</span>}

    <div className='flex gap-2'>
    <p className='text-xl  mt-1'>Resolution:</p>
    <input className="px-3 py-2 border border-gray-700 w-full" {...register('resolution', { required: true })} />
    </div>
